...
#ifndef PIPE
bool CameraSocketServerThread::configureCapabilities(bool skip) {
...
bool CameraSocketServerThread::configureCapabilities(bool skipCapRead) {
    ALOGE(LOG_TAG " %s Enter", __FUNCTION__);
...
if(!skipCapRead) {
        ...
        int headerRead = read(mClientFd, &header, sizeof(camera_header_t));
        ALOGE("header read size %d\n",headerRead);
        if(headerRead < 0) 
            return false;
    if (header.type != REQUEST_CAPABILITY) {
        ALOGE(LOG_TAG "%s: Invalid packet type\n", __FUNCTION__);
    ...
    memcpy(cap_packet->payload, &capability, sizeof(camera_capability_t));
    int bytesRead = 0;
    bytesRead = write(mClientFd, cap_packet, cap_packet_size);
    ALOGE("cap packet size write %d\n",bytesRead);
    if(bytesRead < 0) 
        return false;
    ...
    if(trans_mode == PIPE) {
        recv_size = read(mClientFd, &header, sizeof(camera_header_t));
        ALOGE("cap packet size %d\n",(int)recv_size);
...
//        goto out;
    ...
    if(trans_mode == PIPE) {
        recv_size = read(mClientFd, &camera_info, mNumOfCamerasRequested * sizeof(camera_info_t));
        ALOGE("cam info size %d\n",(int)recv_size);
   ...
   // pthread_mutex_unlock(&gVirtualCameraFactory.mCapReadLock);
    ...
    memcpy(ack_packet->payload, &ack_payload, sizeof(camera_ack_t));
    if(trans_mode == PIPE) {
        recv_size = write(mClientFd, ack_packet, ack_packet_size);
        ALOGE("cap ack size %d\n",(int)recv_size);
    ...
    if(trans_mode == UNIX)
    {
        ...
        while (mRunning) {
            if(trans_mode == PIPE) {
                ...
                while(1) {
                    ALOGD("%s: opening camera data pipe...\n", __FUNCTION__);
            ...
            pthread_cond_signal(&mSignalHotplug);
            int retryLoop = 0;
            while (true) {
